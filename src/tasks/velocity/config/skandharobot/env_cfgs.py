"""Skandha velocity environment configurations."""

from src.assets.robots import (
  SKANDHA_ROBOT_ACTION_SCALE,
  get_skandha_robot_cfg,
)
...
# Skandha's floating base is `base_link` (the torso/chest) -- the pelvis-like `hip_link`
# hangs below it through `waist_joint`, same as kodyrobot. Every "root body" reference
# below (raycast frame, IMU-derived observations via the XML's
# `imu`/`imu_ang_vel`/`imu_lin_vel` sensors, viewer target, CoM randomization,
# orientation/ang-vel rewards) therefore targets `base_link`.
...
def skandha_robot_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Skandha rough terrain velocity configuration."""
  ...
  cfg.scene.entities = {"robot": get_skandha_robot_cfg()}

  # Set raycast sensor frame to Skandha's floating base.
  ...
  joint_pos_action.scale = SKANDHA_ROBOT_ACTION_SCALE
  ...
  # Same std-value philosophy as kodyrobot/unitree_h2/unitree_g1 (same joint-group
  # layout: legs / waist / arms / head):
...
def skandha_robot_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Skandha flat terrain velocity configuration."""
  cfg = skandha_robot_rough_env_cfg(play=play)
